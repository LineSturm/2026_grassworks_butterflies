...
RFmodel_1200 <- randomForest(butterfly.rich.target~., data = train_r1200, 
                                  ntree = 2000)
RFmodel_1200
plot(RFmodel_1200)
ntree_best <- 1000
...
#   mutate(response.var = "rich_target_1200")
# 
# tune_r1200 %>%
#   head(10)
# 
# # save
# tune_r1200 %>%
#   write.xlsx(here("outputs", "models", "cRF", "cRF_mtry", "tune_mtry_rich_target_1200_2026-03-12.xlsx"))

# load 
tune_mtry <- readxl::read_excel(
  here("outputs", "models", "cRF", "cRF_mtry", "tune_mtry_rich_target_1200_2026-03-12.xlsx"))
...
plot_tune_mtry_spe_rich_target <- tune_mtry %>% 
  ...
  labs(x = "mtry", y = "OOB RSME", 
       subtitle = "best mtry = 10, ntree = 1000",
...
# cf_1200 <- party::cforest(formula = butterfly.rich.target ~ .,
#                                data    = train_r1200,
#                                controls = cforest_unbiased(ntree = ntree_best,
#                                                            mtry = mtry_best))
# 
# # save model
# saveRDS(cf_1200, here("outputs", "models", "cRF", "cRF_RData", "cf_1200_rich_target_2026-03-12.RData"))

# load model
cf_1200 <- readRDS(here("outputs", "models", "cRF", "cRF_RData", "cf_1200_rich_target_2026-03-12.RData"))
...
"stats_training_rich_target_1200_2026-03-12.xlsx"))
...
pred_RMSE_1200 <- round(sqrt(mean((pred_1200-test_r1200$butterfly.rich.target)**2)),2)
pred_RMSE_1200
# 3.08
pred_R2_1200 <- round(cor(pred_1200, test_r1200$butterfly.rich.target)**2,2)
pred_R2_1200
# 0.44
...
#          value = round(value, 2))
# imp_r1200 %>% write.xlsx(here("outputs", "tables", "imp_r1200_rich_target.xlsx"))
