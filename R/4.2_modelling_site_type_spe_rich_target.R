#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# GRASSWORKS Project 
# Analyses of site type - target species richness
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# author: Line Sturm
# 13 June 2026
...
## 1 glm1 ####
glm1 <- glm(butterfly.rich.target ~ site.type + obs.year + region + site.cwm.pres.oek.f
            + region:obs.year,
...
## 2 glm1a ####
glm1a <- glm(butterfly.rich.target ~ site.type + obs.year + region + site.cwm.pres.oek.f,
            data = data, family = "poisson")
summary(glm1a)
...
## 3 glm1a_nb ####
glm1a_nb <- glm.nb(butterfly.rich.target ~ site.type + obs.year + region + site.cwm.pres.oek.f,
             data = data)
summary(glm1a_nb)
...
# data:  glm1a_nb
# observed = 0.0804611, expected = -0.0053763, sd = 0.0284053, p-value = 0.002512
...
## 4 gam1 ####
gam1 <- gam(
  butterfly.rich.target ~ site.type + obs.year + region + region:obs.year
  + site.cwm.pres.oek.f 
  + s(x, y), 
  family = poisson(), 
...
## 5 gam1a ####
gam1a <- gam(
  butterfly.rich.target ~ site.type + obs.year + region
  + site.cwm.pres.oek.f 
  + s(x, y), 
  family = poisson(), 
  data = data, method = "REML")
gam1a_summary <- summary.gam(gam1a)
sink(here("outputs", "models", "all_sites", "spe_rich_target",
...
# DHARMa Moran's I test for distance-based autocorrelation
# data:  gam1a
# observed = 0.0182168, expected = -0.0053763, sd = 0.0284013, p-value = 0.4061
# alternative hypothesis: Distance-based autocorrelation


### d) validate model ----------------------------------------------------------
gam.check(gam1a)
png(here("outputs", "models", "all_sites", "spe_rich_target",
...
dat_pred2 %>% write.csv(file = here::here("outputs", "tables", 
                                          "EMM1_site_type_spe_rich_target.csv"), row.names = F)
