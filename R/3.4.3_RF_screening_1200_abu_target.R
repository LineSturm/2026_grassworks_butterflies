...
data_r1200 <- data_restored %>%
  dplyr::select(butterfly.target.abu,
...
RFmodel_1200 <- randomForest(butterfly.target.abu~., data = train_r1200, 
                                  ntree = 2000)
RFmodel_1200
plot(RFmodel_1200)
ntree_best <- 1000
...
#   model <- party::cforest(formula = butterfly.target.abu~.,
#                           data    = train_r1200,
...
#   mutate(response.var = "abu_target_1200")
# 
# tune_r1200 %>%
#   head(10)
# 
# # save
# tune_r1200 %>%
#   write.xlsx(here("outputs", "models", "cRF", "cRF_mtry", "tune_mtry_abu_target_2026-03-12.xlsx"))

# load 
tune_mtry <- readxl::read_excel(
  here("outputs", "models", "cRF", "cRF_mtry", "tune_mtry_abu_target_2026-03-12.xlsx"))
...
plot_tune_mtry_abu_target <- tune_mtry %>% 
  ...
  labs(x = "mtry", y = "OOB RSME", 
       ...
       title = "(D) Target abundance")
print(plot_tune_mtry_abu_target)
mtry_best <- 5
...
# cf_1200 <- party::cforest(formula = butterfly.target.abu ~ .,
#                                data    = train_r1200,
#                                controls = cforest_unbiased(ntree = ntree_best,
#                                                            mtry = mtry_best))
# 
# # save model
# saveRDS(cf_1200, here("outputs", "models", "cRF", "cRF_RData", "cf_1200_abu_target_2026-03-12.RData"))

# load model
cf_1200 <- readRDS(here("outputs", "models", "cRF", "cRF_RData", "cf_1200_abu_target_2026-03-12.RData"))
...
"stats_training_abu_target_1200_2026-03-12.xlsx"))
...
pred_RMSE_1200 <- round(sqrt(mean((pred_1200-test_r1200$butterfly.target.abu)**2)),2)
pred_RMSE_1200
# 16.96
pred_R2_1200 <- round(cor(pred_1200, test_r1200$butterfly.target.abu)**2,2)
pred_R2_1200
# 0.42
...
#          value = round(value, 2))
# imp_r1200 %>% write.xlsx(here("outputs", "tables", "imp_r1200_abu_target.xlsx"))
